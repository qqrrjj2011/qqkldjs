...
namespace GracesGames._2DTileMapLevelEditor.Scripts.Functionalities
{
    ...
    public class LoadFunctionality : MonoBehaviour
    {
        ...
        //自定义的属性数据在这里读取
        private void LoadLevelPropertiesJsonData(String Jsondata)
        {
            JsonObject jsonObject = SimpleJson.SimpleJson.DeserializeObject<JsonObject>(Jsondata);
             _levelEditor.rowStart = int.Parse(jsonObject.GetString("RowStart"));
 
      
             if(!int.TryParse(jsonObject.GetString("ballStartNum"),out _levelEditor.ballStartNum))
                {
                    _levelEditor.ballStartNum = 1;
                }

                if(!float.TryParse(jsonObject.GetString("blockScale"),out _levelEditor.blockScale))
                {
                    _levelEditor.blockScale = 1;
                }
               

                if(!int.TryParse(jsonObject.GetString("maxCol"),out _levelEditor.Width))
                {
                    _levelEditor.Width = 7;
                    GridOverlay.Instance.SetGridSizeX(_levelEditor.Width);
                }

            _levelEditor.UiScript.SetUpPropertiesPlane(jsonObject);
        }
            ...
            switch (_loadMethod)
            {
                ...
                case TileIdentificationMethod.Name:
                    ...
                    for (int i = 0; i < _tiles.Count; i++)
                    {
                        ...
                        {
                            Transform txt = _tiles[i].Find("Sprite").Find("text");
                            if (txt != null)
                            {
                               txt.GetComponent<TextMesh>().text = "" +int.Parse(temp[1]);
                            } 
