...
namespace GracesGames._2DTileMapLevelEditor.Scripts
{
    ...
    // Enum used to define save type 自动存储的数据信息
    ...
    public class LevelEditor : MonoBehaviour
    {
        ...
        public UserInterface UiScript { get { return _uiScript; } }
        ...
        {
            _startPath = Application.dataPath + "/Resources/LevelData/";
        }
        ...
        private void SetupFunctionalities()
        {
            ...
            _saveFunctionality.Setup(FileBrowserPrefab, FileExtensions, SaveMethod, _tiles, _startPath, _gameObjects);
        ...
        private void HandleInput(int posX, int posY)
        {
            ...
            if (Input.GetMouseButton(0) && GUIUtility.hotControl == 0 && !EventSystem.current.IsPointerOverGameObject())
            {
                ...
                if (_selectedTileIndex != Empty)
                {
                    ...
                    // //暂时构建 更新构建物的信息
                    // if (_previewTile.GetComponent<ChunkCollider>() != null)
                    // {
                    //     ChunkCollider chunk = _previewTile.GetComponent<ChunkCollider>();
                    //     if (_uiScript.SocreInput.text != null && !_uiScript.SocreInput.text.Equals(""))
                    //     {
                    //         chunk.initChunk(int.Parse(_uiScript.SocreInput.text));
                    //     }
                    // }
                }
            }
        ...
        //方法，以切换选择瓷砖的选择。
        //在UserInterface脚本中使用clicklistener。
        public void ButtonClick(int tileIndex)
        {
            ...
            _previewTile = Instantiate(GetTiles()[_selectedTileIndex],
               ...
               GetTiles()[_selectedTileIndex].localRotation);
        ...
        //loadfunction 调用 UpdateLayerVisibility，以消除对layerfunction脚本的依赖。
        ...
        private void BuildBlock(Transform toCreate, int xPos, int yPos, int zPos, Transform parent)
        {
            ...
            // //修改构建物本身的属性脚本 暂时构建
            Transform txt = newObject.Find("Sprite").Find("text");
            if ( txt != null)
            {
                if (_uiScript.SocreInput.text != null && !_uiScript.SocreInput.text.Equals(""))
                {
                    txt.GetComponent<TextMesh>().text = _uiScript.SocreInput.text;
                }
