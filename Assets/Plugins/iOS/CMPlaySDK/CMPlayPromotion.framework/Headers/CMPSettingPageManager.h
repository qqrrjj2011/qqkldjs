//
//  CMPSettingPageManager.h
//  WhiteTile
//
//  Created by KepenJ on 16/6/22.
//
//  If you have any questions about CMPlayBase.framework,please contact KepenJ(Email: kepenj@gmail.com )

#import <Foundation/Foundation.h>

@protocol CMPSettingCardClickDelegate;

@interface CMPSettingPageModel : NSObject
//**************这些是解析出来的数据**********
...
@property (nonatomic,copy) NSString *button_txt;        //button test
...
@property (nonatomic,assign) NSInteger isFirstShow;        //当前Arry中的位置
@property (nonatomic,assign) BOOL show_red_dot_setting_card; //是否显示按钮
...
@property (nonatomic,assign) NSInteger currentIndex;        //当前Arry中的位置
...
@interface CMPSettingPageManager : NSObject
/**
 *  当前SettingPageModel
 */
@property (nonatomic,retain,readonly) CMPSettingPageModel * currentSettingPageModel;
/**
 *
 *  回调的代理
 */
@property (strong) id<CMPSettingCardClickDelegate> delegate;
 ...
 *  @return YES/NO
 */
- (BOOL)initSettingPage;
/**
 *  判断是否可以展示设置页Banner
 *
 *  @return YES/NO
 */
- (BOOL)canShowSettingPageScreen;
/**
 *  设置页面红点是否可展示
 *
 *  @return YES/NO
 */
- (BOOL)canShowSettingPageRedDot;
/**
 *  获取当前结果页所展示的数据
 *
 ...
 */
- (NSString *)getJsonStringOfCurrentSettingPageModel;

/**
 *  获取当前结果页所展示的数据
 *
 *  @return 该数据的 Dic 对象(下面是格式！！！！)
 ...
 */
- (NSDictionary*)getDictionaryOfCurrentSettingPageModel;
/**
 *  相应相关的点击事件
 *
 ...
 *  infoc 埋点
 */
- (void)reportInfoc:(CMPSettingPageModel*)info action:(int)action remark:(NSString*)remark scenes:(int)scenes;

@end


@protocol CMPSettingCardClickDelegate <NSObject>
...
- (void)reportSettingPageOther:(CMPSettingPageModel*)info action:(int)action remark:(NSString*)remark scenes:(int)scenes sdkVersion:(int)sdkVersion totalInfo:(NSMutableDictionary *)totalInfo;
@end
